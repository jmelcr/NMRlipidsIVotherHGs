...
\title{Headgroup and glycerol backbone structures, and cation binding in bilayers with PS lipids} %Title of paper
...
Some domains specifically interact with PS lipids,
while others are attracted by general electrostatics 
with a possible regulation by calcium \cite{leventis10}.
...
to a POPC bilayer was then improved by implicitly including the electronic
...
role of PS and other lipid headgroups as glycerol backbone and
...
\caption{List of MD simulations without additional salt.
   ...
   'M' stands for Mukhopadhyay after the first published Berger-based PS simulation that used these charges \cite{mukhopadhyay2004})
...
\begin{tabular}{l c c r r r r r r c c}
 ...
 lipid/counter-ions & force field for lipids / ions & NaCl (mM) & CaCl$_2$\,(mM) &  \footnote{Number of lipid molecules with the largest mole fraction}N$_{\rm l}$   &  \footnote{Number of water molecules}N$_{\rm w}$ \todoi{Should confirm that the amounts of water in experiments matched those in simulations.}   & \footnote{Number of additional cations}N$_{\rm c}$  & \footnote{Simulation temperature}T (K)  & \footnote{Total simulation time}t$_{{\rm sim}}$(ns) & \footnote{Time used for analysis}t$_{{\rm anal}}$ (ns) &   \footnote{Reference for simulation files}files\\
  \hline
    DOPS/Na$^+$  & CHARMM36 \cite{venable13}       &0 & 0 & 128 & 4480 & 0  & 303  & 500 & 100 & \cite{charmm36DOPS303K} \\
    DOPS/Na$^+$  & CHARMM36ua \cite{catte16} \todoi{Correct citation for CHARMMua DOPS}        &0 & 0        & 128 & 4480 & 0  & 303  & 500 & 100 & \cite{charmm36uaDOPS303K} \\
    ...
    DOPS/Na$^+$  & GROMOS-CKP1 \cite{catte16} \todoi{Correct citation(s) for CKP.} &0 & 0  & 128 & 4480 & 0  & 303  & 500 & 100 & \cite{ckp1DOPS303K} \\
    DOPS/Na$^+$  & GROMOS-CKP2 \cite{catte16} \todoi{Correct citation(s) for CKP.} &0 & 0  & 128 & 4480 & 0  & 303  & 500 & 100 & \cite{ckp2DOPS303K} \\
    ...
    POPS/Na$^+$  & CHARMM36ua \cite{catte16} \todoi{Correct citation for CHARMMua DOPS}  &0 & 0 & 128 & 4480 & 0  & 298  & 500 & 100 & \cite{charmm36uaPOPS298K} \\
    POPS/Na$^+$  & Slipids \cite{jambeck13}        &0 & 0        & 128 & 4480 & 0  & 298  & 500 & 100 & \cite{slipidsPOPS298K} \\
    POPS/Na$^+$  & Berger \cite{catte16}        &0 & 0        & 128 & 4480 & 0  & 298  & 500 & 100 & \cite{bergerPOPS298K} \\
%    POPS/Na$^+$  & MacRog \cite{maciejewski14}  &0 & 0        & ? & catte16 & 0  & ?  & ? & ?  & \cite{catte16} \todoi{Data to be added by Piggot} \\
    OPPS/Na$^+$  & MacRog \cite{maciejewski14}  &0 & 0        & 128 & 5120 & 0  & 298  & 200 & 100 & \cite{macrogPOPS298K} \\
    POPS/Na$^+$  & GROMOS-CKPM \cite{catte16} \todoi{Correct citation(s) for CKP.} &0 & 0  & 128 & 4480 & 0  & 298  & 500 & 100 & \cite{ckp1POPS303K} \\
    POPS/Na$^+$  & GROMOS-CKP \cite{catte16} \todoi{Correct citation(s) for CKP.} &0 & 0  & 128 & 4480 & 0  & 298  & 500 & 100 & \cite{ckp2POPS303K} \\
...
%\begin{sidewaystable*}[!p]
\centering
\caption{  List of MD simulations with additional salt concentrations. 
   The salt concentrations were calculated as 
...
%    POPC:POPS (5:1)/K$^+$  & CHARMM36 \cite{klauda10,venable13}          &0 & 0 & 250:50 & ?     & 0  & 298  & 200 & ?   & \cite{catte16} \todoi{Trajectories and further details to be added by J. Madsen}  \\
...
%    POPC:POPS (1:1)/K$^+$  & CHARMM36 \cite{klauda10,venable13}          &0 & 0 & 150:150 & ?    & 0  & 298  & 200 & ?   & \cite{catte16} \todoi{Trajectories and further details to be added by J. Madsen}  \\
%     POPC:POPS (5:1)        & CHARMM36 \cite{klauda10,venable13,kim16}   &0 & 150 \todoi{Concentration to be checked} & 250:50 & ?  & ?  & 298  & 200 & ?  & \cite{catte16} \todoi{Trajectories and further details to be added by J. Madsen}  \\
%    POPC:POPS (5:1)        & CHARMM36 \cite{klauda10,venable13,kim16}    &0 & 1000 \todoi{Concentration to be checked} & 250:50 & ?  & ?  & 298  & 200 & ?  & \cite{catte16} \todoi{Trajectories and further details to be added by J. Madsen}  \\
    \hline
...
The simulated systems are listed in Tables \ref{IONsystems} and \ref{IONsystems_conc}  and
simulation details are given in the SI.
The simulation data are also indexed in the
...
decrease proportionally to the amount of bound positive
charge to a bilayer \cite{akutsu81,altenbach84,seelig87}.
This molecular electrometer concept is especially useful for 
the comparison between simulations and experiments as
the headgroup order parameters can be directly obtained from 
the both of them~\cite{catte16}. Also the headgroup order parameters
...
mixed bilayers (see supplementary information).
...
The INEPT and 2D R-PDLF experiments from POPS samples give well resolved spectra for all the
...
\begin{figure}[!htb]
  ...
  \caption{\label{PShgSIGNSsimpson}
    ...
    for the larger and smaller $\alpha$-carbon splittings were used in the
...
\begin{figure}[]
  ...
  \caption{\label{HGorderParameters}
    ...
    as measured in this work and signs for PC as measured in Refs \cite{botan15, ferreira16}.
...
\begin{figure}[]
  ...
  \caption{\label{HGorderParametersPS}
    ...
    we had at least two data sets; the ends of the bars mark the extreme values from the sets, and the dots mark their time-weighted average.
  }
\end{figure}
...
which both reproduce the larger forking of the $\alpha$-carbon, 
with Slipids reproducing also the lower order parameter of the $\beta$-carbon 
in the PS headgroups (Fig.~\ref{HGorderParametersPS} and Fig.~2 in Ref.~\citenum{botan15}).
...
and its conformational differences  %are illustrated in Figure \ref{glycerol_buslaev}. 
(see supplementary information). 
...
in MD simulations and, hence, to reduce the area per lipid of PS bilayers to be smaller than in PC
...
The difference cannot be explained solely by the electrostatic screening of the headgroup repulsion due to 
...
This is evident also in the changes of PS headgroup order parameter $\beta$, 
which decreases with the addition of lithium 
but increases with the addition of sodium or potassium (Fig. \ref{PSresponseTONaCl}), 
while changes of the order parameter $\alpha$ are in the opposite direction. 
...
\begin{figure*}[]
  ...
  \caption{\label{PSresponseTONaCl}
    ...
    added NaCl, KCl and LiCl from POPC:POPS (5:1) mixture. The experimental data are from Ref.~\citenum{roux90}.
    The values from counterion-only systems are set as a zero point of the y-axis.
...
\begin{figure}[]
  ...
  \caption{  Density distributions of counterions from PC:PS mixtures.
...
\begin{figure*}[]
  ...
  \caption{\label{HGorderparametersPCvsPS}
    ...
    different experimental sets ($^{13}$C NMR in this work and $^{2}$H NMR from Ref. \citenum{roux90}),
...
parameters from Ref. \citenum{kim16}, incorporated in the parameters given by
...
\begin{figure*}[]
  ...
  \caption{\label{changesWITHCaClPS}
    ...
    as a function CaCl$_2$ concentration. Experimental data are taken from Ref.~\citenum{roux90}.
    The values from counterion-only systems are set as a zero point of the y-axis.
    To correctly illustrate the significant forking of the $\alpha$-carbon order parameter
    in PS headgroup (bottom, right), the y-axis is tranferred with the same value for both order parameters such that the lower order
    parameter value is at zero. 
...
\begin{figure}[]
  ...
  \caption{\label{CAdensPCPSmixture}
     Density profiles of Ca$^{2+}$ from simulations.
  }
...
less well understood than the response of PC headgroups used in the electrometer
concept \cite{seelig87,roux90}. 
Additional force field development is necessary to
generate MD simulations that could be used to explain the interactions between
PS headgroups and calcium ions.
...
MD simulation models seem to generally overestimate cation binding also
...
We have collected 
 sets of experimental data, which are highly useful in the quality assessment, 
and which are easily accessible through the open collaboration platform \url{nmrlipids.blogspot.fi}. 
The pinpointed problematic areas in the models suggest possible directions for corrections.
Such improvements are already in progress \url{https://github.com/jmelcr/ecc_lipids}
following the recent work on PC lipids \cite{melcr18}, 
which uses the electronic continuum correction to account for electronic polarization.
